...
Usa la libreria `soundcard` (loopback WASAPI nativo de Windows), por lo que
funciona en cualquier Windows 10/11 sin "Stereo Mix" ni dispositivos virtuales.

Es BEST-EFFORT: cualquier fallo se registra y NO interrumpe la grabacion de
video (el video siempre se guarda; como mucho se queda sin pista de audio).
"""
...
def list_microphones() -> list[str]:
    """Nombres de microfonos de entrada (sin loopback)."""
    if not AVAILABLE:
        return []
    try:
...
class AudioCapture:
    ...
    def _resolve_tracks(self) -> list[_Track]:
        ...
        if self.system:
            try:
                ...
                tracks.append(_Track(lb, str(Path(self.work_dir) / ".capturapro_sys.wav"), "system"))
            except Exception as exc:  # noqa: BLE001
        ...
        if self.mic_name:
            try:
                ...
                tracks.append(_Track(mic, str(Path(self.work_dir) / ".capturapro_mic.wav"), "mic"))
            except Exception as exc:  # noqa: BLE001
                ...
                with track.device.recorder(samplerate=SAMPLERATE,
                    ...
                    while not self._stop.is_set():
                        ...
                        if self._paused.is_set():
                            continue  # durante la pausa se descarta (no se graba)
    ...
    def stop(self) -> list[str]:
        """Detiene la captura y devuelve las rutas de los WAV validos."""
        self._stop.set()
        for t in self._tracks:
            if t.thread:
    ...
    def cleanup(self) -> None:
        """Borra los WAV temporales (en caso de fallo del video)."""
        for t in self._tracks:
            try:
